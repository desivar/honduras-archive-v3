...
export default function TodayInHistory() {
  ...
  <style>{`
    ...
    .tih-record-card:hover {
      background: rgba(69, 172, 236, 0.4) !important;
      transform: translateY(-2px);
      box-shadow: 0 4px 16px rgba(0,0,0,0.3) !important;
    }
    .tih-record-card { transition: all 0.25s ease; }
    .tih-dot:hover { opacity: 1; }
  `}</style>
  ...
  <div style={{
    ...
    <div style={{
      ...
      boxShadow: '0 8px 40px rgba(0,0,0,0.5), inset 0 1px 0 rgba(234,198,61,0.3)',
      border: '1px solid rgba(234,198,61,0.4)',
    }}>
      ...
      {['top:12px;left:14px','top:12px;right:14px;transform:scaleX(-1)',
        ...
        <div key={i} style={{
          position: 'absolute', color: 'rgba(234, 198, 61, 0.85)',
      ...
      <div style={{ position: 'relative', zIndex: 3, padding: '28px 32px 24px' }}>
        ...
        {/* Header image */}
        <div style={{ width: '100%', marginBottom: '20px', borderRadius: '8px', overflow: 'hidden', border: '2px solid rgba(234,198,61,0.5)' }}>
          <img src="/white.png" alt="Archivo Histórico de Honduras"
            style={{ width: '100%', height: '200px', objectFit: 'cover', display: 'block', opacity: 0.95 }} />
        </div>

        {/* Date header */}
        <div style={{ textAlign: 'center', marginBottom: '20px', animation: 'fadeInDown 0.8s ease both' }}>
          <p style={{ margin: '0 0 4px', fontFamily: "'Cormorant Garamond', serif", fontSize: '0.75rem', letterSpacing: '0.28em', textTransform: 'uppercase', color: '#F1D776', fontWeight: 600 }}>
            ── Archivo de Honduras ──
          </p>
          <h2 style={{ margin: 0, fontFamily: "'Playfair Display', serif", fontSize: 'clamp(1.6rem, 3vw, 2.4rem)', fontWeight: 700, color: '#FFFFFF', textShadow: '0 2px 12px rgba(0,0,0,0.6)', lineHeight: 1.1 }}>
            Registro del Día
          </h2>
          <p style={{ margin: '6px 0 0', fontFamily: "'IM Fell English', serif", fontStyle: 'italic', fontSize: 'clamp(1.05rem, 2vw, 1.25rem)', color: '#F8E8A0', letterSpacing: '0.04em' }}>
            {getTodayLabel()}
          </p>
          {/* Gold divider */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', marginTop: '14px' }}>
            <div style={{ height: '1px', width: '60px', background: 'linear-gradient(to right, transparent, rgba(234,198,61,0.9))' }} />
            <span style={{ color: '#F1D776', fontSize: '0.75rem' }}>✦</span>
            <div style={{ height: '1px', width: '60px', background: 'linear-gradient(to left, transparent, rgba(234,198,61,0.9))' }} />
          </div>
        </div>
        ...
        <div style={{ textAlign: 'center', marginBottom: '24px', minHeight: '90px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', animation: 'fadeInUp 1s ease 0.2s both' }}>
          <p className="tih-quote-fade" style={{ opacity: fadeQuote ? 1 : 0, margin: '0 0 10px', fontFamily: "'IM Fell English', serif", fontStyle: 'italic', fontSize: 'clamp(1rem, 1.8vw, 1.15rem)', color: '#FFFFFF', lineHeight: 1.65, maxWidth: '540px', textShadow: '0 1px 8px rgba(0,0,0,0.6)' }}>
            "{q.quote}"
          </p>
          <p className="tih-quote-fade" style={{ opacity: fadeQuote ? 1 : 0, margin: 0, fontFamily: "'Cormorant Garamond', serif", fontSize: '0.85rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: '#F1D776', fontWeight: 600 }}>
            — {q.author}, {q.year}
          </p>
          {/* Quote dots */}
          <div style={{ display: 'flex', gap: '6px', marginTop: '12px' }}>
            {HISTORICAL_QUOTES.map((_, i) => (
              <div key={i} className="tih-dot"
                ...
                style={{ width: i === quoteIndex ? '20px' : '6px', height: '6px', borderRadius: '3px', background: i === quoteIndex ? '#F1D776' : 'rgba(234,198,61,0.4)', cursor: 'pointer', transition: 'all 0.3s ease' }} />
            ))}
          </div>
        </div>
        ...
        <div style={{ animation: 'fadeInUp 1s ease 0.5s both' }}>
          <p style={{ margin: '0 0 12px', fontFamily: "'Cormorant Garamond', serif", fontSize: '0.75rem', letterSpacing: '0.22em', textTransform: 'uppercase', color: '#F1D776', textAlign: 'center', fontWeight: 600 }}>
            ✦ Registro Destacado de Hoy ✦
          </p>

          {loading && (
            <p style={{ textAlign: 'center', fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic', fontSize: '0.9rem', color: '#F8E8A0' }}>
              Cargando registro...
            </p>
          )}
          ...
          {!loading && record && (
            <div
              ...
              style={{
                ...
                border: '1px solid rgba(234,198,61,0.35)',
                overflow: 'hidden',
                cursor: 'pointer',
              }}
            >
            ...
            borderBottom: '1px solid rgba(234,198,61,0.3)', 
            display: 'flex', 
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <span style={{
                      ...
                      background: 'rgba(46,126,50,0.4)',
                      border: '1px solid rgba(76,175,80,0.7)',
                      fontFamily: "'Cormorant Garamond', serif",
                      fontSize: '0.75rem', letterSpacing: '0.15em',
                      textTransform: 'uppercase', color: '#C8FACB',
                      fontWeight: 600,
                    }}>
                      {record.category}
                    </span>
                    ...
                    {/* Name */}
                    <p style={{ margin: '0 0 4px', fontFamily: "'Playfair Display', serif", fontSize: '0.95rem', fontWeight: 700, color: '#FFFFFF', lineHeight: 1.3 }}>
                      {displayName}
                    </p>

                    {/* Date & location */}
                    <p style={{ margin: '0 0 6px', fontFamily: "'Cormorant Garamond', serif", fontStyle: 'italic', fontSize: '0.82rem', color: '#BDE0FE' }}>
                      {[record.eventDate || record.publicationDate, record.location, record.newspaperName].filter(Boolean).join(' · ')}
                    </p>

                    {/* Summary */}
                    {record.summary && (
                      <p style={{ margin: 0, fontFamily: "'Cormorant Garamond', serif", fontSize: '0.88rem', color: '#E2EEF9', lineHeight: 1.5, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                        {record.summary}
                      </p>
                    )}
                  </div>
                  <span style={{ color: '#F1D776', fontSize: '1.2rem', marginLeft: '12px', marginTop: '4px' }}>›</span>
                </div>

                {/* View link */}
                <div style={{ marginTop: '8px', paddingTop: '8px', borderTop: '1px solid rgba(234,198,61,0.25)', textAlign: 'right' }}>
                  <span style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '0.82rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: '#F1D776', fontWeight: 600 }}>
